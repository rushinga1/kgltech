  ...
  <div class="space-y-32 pb-20">
        ...
        <div class="relative z-10 animate-float">
          <div class="glass flex flex-col gap-6 rounded-[2.5rem] p-10">
            <div class="flex items-start gap-5">
              <div class="flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl bg-brand-blue/20 text-2xl">⚡</div>
              <div>
                <p class="text-sm font-bold uppercase tracking-widest text-brand-blue">Swift Delivery</p>
                <p class="mt-1 text-xl font-bold">5-7 Business Days</p>
                <p class="text-sm text-slate-400">Doorstep delivery across Kigali province</p>
              </div>
            </div>
            <div class="h-px w-full bg-white/10"></div>
            <div class="flex items-start gap-5">
              <div class="flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl bg-brand-green/20 text-2xl">🌍</div>
              <div>
                <p class="text-sm font-bold uppercase tracking-widest text-brand-green">Global Brands</p>
                <p class="mt-1 text-xl font-bold">100% Genuine Gear</p>
    ...
    <section class="page-shell">
      <div class="mb-12 flex items-end justify-between">
        <div>
          ...
          <h2 class="mt-3 text-4xl font-black tracking-tight text-slate-900">Trending Tech Picks</h2>
        </div>
    ...
    <section class="page-shell">
      <div class="mb-12 text-center">
        ...
        <h2 class="mt-3 text-4xl font-black tracking-tight text-slate-900">Explore Collections</h2>
      </div>
      <div class="grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
        <RouterLink
          ...
          class="panel group relative flex flex-col items-center gap-6 p-10 text-center transition-all hover:border-brand-blue/30 lg:p-12"
        >
          <div class="absolute inset-0 z-0 origin-center scale-90 rounded-[2.5rem] bg-brand-blue/5 opacity-0 transition-all group-hover:scale-100 group-hover:opacity-100"></div>
          <span class="relative z-10 text-6xl transition-transform duration-500 group-hover:scale-110">{{ categoryIcons[category] || '🛍️' }}</span>
          <div class="relative z-10">
            <p class="text-xl font-black capitalize text-slate-900">{{ category }}</p>
            <p class="mt-1 text-sm font-medium text-slate-500">Shop Premium</p>
          </div>
        </RouterLink>
      </div>
    </section>
    ...
    <!-- Testimonials -->
    <section class="page-shell rounded-[4rem] bg-slate-100/50 py-24">
      <div class="mb-16 px-6 text-center">
        ...
        <h2 class="mt-3 text-4xl font-black tracking-tight text-slate-900">Trusted by Kigali Techies</h2>
      </div>
      <div class="grid gap-8 lg:grid-cols-3">
        <article v-for="testimonial in testimonials" :key="testimonial.name" class="panel p-10 shadow-none ring-1 ring-slate-200/60">
          <div class="flex items-center gap-5">
            ...
            <div>
              <p class="font-black text-slate-900">{{ testimonial.name }}</p>
              <p class="text-xs font-bold uppercase tracking-widest text-slate-400">{{ testimonial.location }}</p>
            </div>
          </div>
          <p class="mt-6 flex gap-1 text-sm text-brand-gold">
            <span v-for="i in 5" :key="i" class="text-lg">{{ i <= testimonial.rating ? '★' : '☆' }}</span>
          </p>
          <p class="mt-5 text-lg leading-relaxed text-slate-600">"{{ testimonial.text }}"</p>
        </article>
      </div>
    </section>
        ...
        <div class="relative z-10 grid gap-12 lg:grid-cols-2 lg:items-center">
          ...
          <form class="flex flex-col gap-4 sm:flex-row" @submit.prevent="email = ''">
            <input
              v-model="email"
              type="email"
              class="w-full rounded-2xl border border-white/10 bg-white/10 px-6 py-5 text-white placeholder:text-white/40 focus:bg-white/20 focus:ring-0 sm:min-w-[300px]"
